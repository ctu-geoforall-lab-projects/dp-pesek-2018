...
The third step is scoring those extracted features (deciding which class the 
feature is and whether it is any class at all). This is done for each feature 
separately using a \zk{SVM} trained for that class.
...
mean average precision (\zk{mAP}) of $53.3 \%$.}, there were still some 
...
The first issue, the separate forward propagations for all region proposals, was 
...
fixed-length feature vector by a region of interest (\zk{RoI}) pooling layer.
...
\zk{SVM}) and bounding box regression as parallel layers to the end of the 
model.
...
There can be raised a question whether is the \zk{SVM} replacement with the 
...
by 0.1 to 0.8 \zk{mAP} points depending on the depth of the 
...
The Fully convolutional network (\zk{FCN}) was proposed by Jonathan Long and the 
...
So we had \zk{R-CNN}, Fast \zk{R-CNN} and Faster \zk{R-CNN}. What could be the 
...
year 2017 by Facebook AI Research \zk{FAIR} in \cite{mask-rcnn}.
...
need for some changes in the architecture of Faster \zk{R-CNN}. Following the 
...
network (\zk{FPN}), a top-down architecture with skip connections (in the 
...
\zk{FPN} is created from output of the last layer of each stage except of the 
...
each \zk{RoI}, where mask is a binary matrix of ones (object location) and zeros 
(elsewhere).
